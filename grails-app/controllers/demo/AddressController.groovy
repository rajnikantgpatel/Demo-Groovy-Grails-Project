...
class AddressController {
	...
	def beforeInterceptor = { println "Executing action Started   : " + new Date() + " $actionName with params $params " }
	def afterInterceptor = 	{ println "Executing action Stopped   : " + new Date() + " $actionName with params $params " }
		...
		request.withFormat {
			form {
				flash.message = message(code: 'default.created.message', args: [
					message(code: 'addressInstance.label', default: 'Address'),
					addressInstance.id
				])
	...
	def upload = {
		...
		if(file && !file.empty && file.size < 1024.multiply(1024)) {
			file.transferTo( new java.io.File( "//home/systemin/${file.name}" ) )
		}
	}
}
